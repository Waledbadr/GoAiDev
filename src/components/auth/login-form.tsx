...
import { Mail, Lock, Eye, EyeOff, KeyRound, Link2, Shield, User, Home, Boxes, FileText, Clock, Wallet } from "lucide-react";
...
export default function LoginForm() {
  ...
  const [appChoice, setAppChoice] = useState<"accommodation" | "materials" | "contracts" | "timesheet" | "finance">(() => {
    if (typeof window === 'undefined') return 'accommodation';
    const saved = window.localStorage.getItem('preferred-app') as any;
    return (saved === 'materials' || saved === 'accommodation' || saved === 'contracts' || saved === 'timesheet' || saved === 'finance') ? saved : 'accommodation';
  });
  ...
  const redirectAfterLogin = (fallbackChoice?: "accommodation" | "materials" | "contracts" | "timesheet" | "finance") => {
    ...
    const target = choice === 'materials' ? '/inventory' : choice === 'contracts' ? '/contracts' : choice === 'timesheet' ? '/timesheet' : choice === 'finance' ? '/income-expenses' : '/accommodation';
    ...
    getRedirectResult(auth)
      .then(async (res) => {
        if (res && res.user) {
          ...
          let persisted: "accommodation" | "materials" | "contracts" | "timesheet" | "finance" | null = null;
          try {
            const saved = typeof window !== 'undefined' ? window.localStorage.getItem('preferred-app') : null;
            if (saved === 'materials' || saved === 'accommodation' || saved === 'contracts' || saved === 'timesheet' || saved === 'finance') persisted = saved as any;
          } catch {}
  ...
  const handleMagicLink = async () => {
    ...
    try {
      ...
      window.localStorage.setItem('pendingEmailForLink', targetEmail);
  ...
  const handlePasskeyLogin = async () => {
    if (!auth) return setError('Authentication is not configured.');
    setLoading(true);
    const targetEmail = toASCII(email).trim() || 'passkey.user@estatecare.app';
    try {
          ...
          <div className="grid gap-2">
            ...
            <div className="grid grid-cols-5 gap-1 rounded-lg bg-muted p-1 text-xs">
              <Button
                ...
                className="rounded-md h-9 px-0"
                onClick={() => setAppChoice('accommodation')}
                title="Accommodation / الإسكان"
              >
              ...
              <Button
                ...
                title="Materials / المواد والعيان"
              >
              ...
              <Button
                type="button"
                size="sm"
                variant={appChoice === 'contracts' ? 'default' : 'ghost'}
                className="rounded-md h-9 px-0"
                onClick={() => setAppChoice('contracts')}
              ...
              <Button
                type="button"
                size="sm"
                variant={appChoice === 'timesheet' ? 'default' : 'ghost'}
                className="rounded-md h-9 px-0"
                onClick={() => setAppChoice('timesheet')}
                title="Timesheet / سجل الحضور"
              >
              ...
              <Button
                type="button"
                size="sm"
                variant={appChoice === 'finance' ? 'default' : 'ghost'}
                className="rounded-md h-9 px-0"
                onClick={() => setAppChoice('finance')}
                title="Income & Expenses / الإيرادات والمصروفات"
              >
